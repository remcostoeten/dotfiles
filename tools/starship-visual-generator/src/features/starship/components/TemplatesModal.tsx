...
export function TemplatesModal({ open, onClose }: TProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-haptic.surface dark:text-haptic.text rounded-lg p-6 max-w-3xl w-full mx-4 max-h-[80vh] overflow-y-auto border dark:border-haptic.border">
        ...
        <div className="space-y-3">
          {templates.map(function render(t){
            return (
              <div key={t.path} className="p-3 rounded border dark:border-haptic.border">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-medium">{t.name}</div>
                    <div className="text-xs text-gray-500">{t.path}</div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button onClick={function on(){ handleApply(t); }} className="px-3 py-1 rounded bg-blue-600 text-white text-sm">Load</button>
                    <button onClick={function on(){ handleEdit(t); }} className="px-3 py-1 rounded border text-sm">Edit</button>
                  </div>
                </div>
              </div>
            ) as any;
