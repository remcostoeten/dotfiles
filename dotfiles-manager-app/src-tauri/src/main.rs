// Prevents additional console window on Windows in release, DO NOT REMOVE!!
#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]

use std::path::PathBuf;
use std::fs;
use std::process::{Command, Stdio};
use std::io::{BufRead, BufReader};
use regex::Regex;
use serde::{Deserialize, Serialize};
use chrono::{DateTime, Utc};
use tar::Builder;
use flate2::Compression;
use flate2::write::GzEncoder;
use tauri::Emitter;

#[derive(Debug, Serialize, Deserialize)]
struct PackageArray {
    name: String,
    packages: Vec<String>,
}

#[derive(Debug, Serialize, Deserialize)]
struct Alias {
    name: String,
    path: String,
    content: String,
}

#[derive(Debug, Serialize, Deserialize)]
struct FileInfo {
    path: String,
    name: String,
    #[serde(rename = "type")]
    file_type: String,
}

#[derive(Debug, Serialize, Deserialize)]
struct GitStatus {
    branch: String,
    status: String,
    changes: GitStatusChanges,
}

#[derive(Debug, Serialize, Deserialize)]
struct GitStatusChanges {
    modified: Vec<String>,
    added: Vec<String>,
    deleted: Vec<String>,
    untracked: Vec<String>,
}

#[derive(Debug, Serialize, Deserialize, Clone)]
struct OutputLine {
    #[serde(rename = "type")]
    line_type: String,
    message: String,
    raw: String,
}

#[derive(Debug, Serialize, Deserialize, Clone)]
struct Backup {
    id: String,
    name: String,
    timestamp: String,
    size: u64,
    path: String,
}

#[derive(Debug, Serialize, Deserialize)]
struct PackageStatus {
    name: String,
    display_name: String,
    installed: bool,
    version: Option<String>,
}

#[derive(Debug, Serialize, Deserialize)]
struct SetupSection {
    name: String,
    description: String,
    packages: Vec<PackageStatus>,
    estimated_time: u32,
}

fn get_dotfiles_path_internal() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "~".to_string());
    PathBuf::from(home).join(".config").join("dotfiles")
}

fn parse_setup_sh() -> Vec<PackageArray> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh_path = dotfiles_path.join("setup.sh");
    
    if !setup_sh_path.exists() {
        return vec![];
    }

    let content = fs::read_to_string(&setup_sh_path).unwrap_or_default();
    let mut arrays = Vec::new();

    // Regex to match array declarations
    let array_regex = Regex::new(r#"declare\s+-a\s+(\w+)=\(([^)]+)\)"#).unwrap();
    let array_assoc_regex = Regex::new(r#"declare\s+-A\s+(\w+)=\(([^)]+)\)"#).unwrap();

    for cap in array_regex.captures_iter(&content) {
        let name = cap.get(1).unwrap().as_str().to_string();
        let packages_str = cap.get(2).unwrap().as_str();
        
        let packages: Vec<String> = packages_str
            .lines()
            .map(|line| line.trim().trim_matches('"').to_string())
            .filter(|line| !line.is_empty() && !line.starts_with('#'))
            .collect();

        arrays.push(PackageArray { name, packages });
    }

    for cap in array_assoc_regex.captures_iter(&content) {
        let name = cap.get(1).unwrap().as_str().to_string();
        let packages_str = cap.get(2).unwrap().as_str();
        
        let packages: Vec<String> = packages_str
            .lines()
            .map(|line| line.trim().trim_matches('"').to_string())
            .filter(|line| !line.is_empty() && !line.starts_with('#'))
            .collect();

        arrays.push(PackageArray { name, packages });
    }

    arrays
}

// Parse ANSI color codes and extract message type
fn parse_output_line(line: &str) -> OutputLine {
    // Remove ANSI color codes
    let ansi_regex = Regex::new(r"\x1b\[[0-9;]*m").unwrap();
    let clean_line = ansi_regex.replace_all(line, "").to_string();
    
    // Detect message type based on content
    let line_type = if clean_line.contains("✓") || clean_line.contains("[SUCCESS]") {
        "success"
    } else if clean_line.contains("✗") || clean_line.contains("[ERROR]") || clean_line.contains("Failed") {
        "error"
    } else if clean_line.contains("⚠") || clean_line.contains("[WARNING]") {
        "warning"
    } else if clean_line.contains("→") || clean_line.contains("[STATUS]") {
        "status"
    } else if clean_line.contains("ℹ") || clean_line.contains("[INFO]") {
        "info"
    } else if clean_line.contains("[DRY RUN]") {
        "dry-run"
    } else {
        "output"
    };
    
    OutputLine {
        line_type: line_type.to_string(),
        message: clean_line.clone(),
        raw: line.to_string(),
    }
}

#[tauri::command]
fn get_package_arrays() -> Result<Vec<PackageArray>, String> {
    Ok(parse_setup_sh())
}

#[tauri::command]
fn add_package_to_array(
    array_name: String,
    package_name: String,
) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh_path = dotfiles_path.join("setup.sh");
    
    if !setup_sh_path.exists() {
        return Err("setup.sh not found".to_string());
    }

    let content = fs::read_to_string(&setup_sh_path)
        .map_err(|e| format!("Failed to read setup.sh: {}", e))?;

    // Format: package_name:Display Name (already formatted by frontend)
    let entry = format!("\"{}\"", package_name);
    
    // Find the array declaration and add the package
    let array_pattern = format!(r#"declare\s+-a\s+{}=\(([^)]+)\)"#, array_name);
    let re = Regex::new(&array_pattern).map_err(|e| format!("Regex error: {}", e))?;
    
    let new_content = re.replace(&content, |caps: &regex::Captures| {
        let existing = caps.get(1).unwrap().as_str();
        let new_packages = if existing.trim().is_empty() {
            format!("    {}", entry)
        } else {
            format!("{},\n    {}", existing.trim_end(), entry)
        };
        format!("declare -a {}={}(\n{}\n)", array_name, "{", new_packages)
    }).to_string();

    fs::write(&setup_sh_path, new_content)
        .map_err(|e| format!("Failed to write setup.sh: {}", e))?;

    Ok(())
}

#[tauri::command]
fn remove_package_from_array(array_name: String, package_index: usize) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh_path = dotfiles_path.join("setup.sh");
    
    if !setup_sh_path.exists() {
        return Err("setup.sh not found".to_string());
    }

    let content = fs::read_to_string(&setup_sh_path)
        .map_err(|e| format!("Failed to read setup.sh: {}", e))?;

    let array_pattern = format!(r#"declare\s+-a\s+{}=\(([^)]+)\)"#, array_name);
    let re = Regex::new(&array_pattern).map_err(|e| format!("Regex error: {}", e))?;
    
    let new_content = re.replace(&content, |caps: &regex::Captures| {
        let existing = caps.get(1).unwrap().as_str();
        let lines: Vec<&str> = existing
            .lines()
            .map(|l| l.trim())
            .filter(|l| !l.is_empty())
            .collect();
        
        if package_index >= lines.len() {
            return format!("declare -a {}={}(\n{}\n)", array_name, "{", existing);
        }

        let mut new_lines = lines.clone();
        new_lines.remove(package_index);
        
        let new_packages = new_lines
            .iter()
            .map(|l| format!("    {}", l))
            .collect::<Vec<_>>()
            .join("\n");

        format!("declare -a {}={}(\n{}\n)", array_name, "{", new_packages)
    }).to_string();

    fs::write(&setup_sh_path, new_content)
        .map_err(|e| format!("Failed to write setup.sh: {}", e))?;

    Ok(())
}

#[tauri::command]
fn get_aliases() -> Result<Vec<Alias>, String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let aliases_dir = dotfiles_path.join("configs").join("fish").join("aliases");
    
    if !aliases_dir.exists() {
        return Ok(vec![]);
    }

    let mut aliases = Vec::new();
    
    if let Ok(entries) = fs::read_dir(&aliases_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("fish") {
                let name = path.file_stem()
                    .and_then(|s| s.to_str())
                    .unwrap_or("unknown")
                    .to_string();
                let path_str = path.to_string_lossy().to_string();
                
                aliases.push(Alias {
                    name,
                    path: path_str,
                    content: String::new(),
                });
            }
        }
    }

    Ok(aliases)
}

#[tauri::command]
fn read_file(path: String) -> Result<String, String> {
    fs::read_to_string(&path)
        .map_err(|e| format!("Failed to read file: {}", e))
}

#[tauri::command]
fn write_file(path: String, content: String) -> Result<(), String> {
    fs::write(&path, content)
        .map_err(|e| format!("Failed to write file: {}", e))
}

#[tauri::command]
fn get_functions() -> Result<Vec<Alias>, String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let functions_dir = dotfiles_path.join("configs").join("fish").join("functions");
    
    if !functions_dir.exists() {
        return Ok(vec![]);
    }

    let mut functions = Vec::new();
    
    if let Ok(entries) = fs::read_dir(&functions_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("fish") {
                let name = path.file_stem()
                    .and_then(|s| s.to_str())
                    .unwrap_or("unknown")
                    .to_string();
                let path_str = path.to_string_lossy().to_string();
                
                functions.push(Alias {
                    name,
                    path: path_str,
                    content: String::new(),
                });
            }
        }
    }

    Ok(functions)
}

#[tauri::command]
fn get_git_status() -> Result<GitStatus, String> {
    let dotfiles_path = get_dotfiles_path_internal();
    
    // Get current branch
    let branch_output = Command::new("git")
        .arg("branch")
        .arg("--show-current")
        .current_dir(&dotfiles_path)
        .output()
        .map_err(|e| format!("Failed to get branch: {}", e))?;
    
    let branch = String::from_utf8_lossy(&branch_output.stdout).trim().to_string();
    
    // Get status
    let status_output = Command::new("git")
        .arg("status")
        .arg("--porcelain")
        .current_dir(&dotfiles_path)
        .output()
        .map_err(|e| format!("Failed to get status: {}", e))?;
    
    let status_lines: Vec<String> = String::from_utf8_lossy(&status_output.stdout)
        .lines()
        .map(|s| s.to_string())
        .collect();
    
    let mut changes = GitStatusChanges {
        modified: Vec::new(),
        added: Vec::new(),
        deleted: Vec::new(),
        untracked: Vec::new(),
    };
    
    for line in status_lines {
        let trimmed = line.trim();
        if trimmed.is_empty() {
            continue;
        }
        
        // Git status format: XY filename
        // X = index status, Y = working tree status
        if trimmed.len() < 3 {
            continue;
        }
        
        let status = &trimmed[..2];
        let filename = trimmed[3..].trim().to_string();
        
        if filename.is_empty() {
            continue;
        }
        
        match status {
            s if s == " M" || s == "MM" || s == "M " || s == "MD" || s == "M!" => {
                changes.modified.push(filename);
            }
            s if s == "A " || s == "AM" || s == "AD" => {
                changes.added.push(filename);
            }
            s if s == "D " || s == "DM" || s == "DA" => {
                changes.deleted.push(filename);
            }
            s if s == "??" => {
                changes.untracked.push(filename);
            }
            s if s.chars().nth(1) == Some('M') => {
                // Any status with M in second position (working tree modified)
                changes.modified.push(filename);
            }
            _ => {}
        }
    }
    
    Ok(GitStatus {
        branch,
        status: String::from_utf8_lossy(&status_output.stdout).to_string(),
        changes,
    })
}

#[tauri::command]
fn run_git_command(command: String, args: Vec<String>) -> Result<String, String> {
    let dotfiles_path = get_dotfiles_path_internal();
    
    let output = Command::new("git")
        .arg(&command)
        .args(&args)
        .current_dir(&dotfiles_path)
        .output()
        .map_err(|e| format!("Git command failed: {}", e))?;
    
    if !output.status.success() {
        let error = String::from_utf8_lossy(&output.stderr);
        return Err(format!("Git error: {}", error));
    }
    
    Ok(String::from_utf8_lossy(&output.stdout).to_string())
}

#[tauri::command]
fn list_files(path: String) -> Result<Vec<FileInfo>, String> {
    let path_buf = PathBuf::from(&path);
    
    if !path_buf.exists() {
        return Err("Path does not exist".to_string());
    }

    let mut files = Vec::new();
    
    if path_buf.is_dir() {
        if let Ok(entries) = fs::read_dir(&path_buf) {
            for entry in entries.flatten() {
                let entry_path = entry.path();
                let file_type = if entry_path.is_dir() {
                    "directory"
                } else {
                    "file"
                };
                
                files.push(FileInfo {
                    path: entry_path.to_string_lossy().to_string(),
                    name: entry_path.file_name()
                        .and_then(|s| s.to_str())
                        .unwrap_or("unknown")
                        .to_string(),
                    file_type: file_type.to_string(),
                });
            }
        }
    } else {
        files.push(FileInfo {
            path: path_buf.to_string_lossy().to_string(),
            name: path_buf.file_name()
                .and_then(|s| s.to_str())
                .unwrap_or("unknown")
                .to_string(),
            file_type: "file".to_string(),
        });
    }

    Ok(files)
}

#[tauri::command]
fn get_dotfiles_path() -> Result<String, String> {
    Ok(get_dotfiles_path_internal().to_string_lossy().to_string())
}

#[tauri::command]
fn open_in_github(path: String) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let relative_path = PathBuf::from(&path)
        .strip_prefix(&dotfiles_path)
        .ok()
        .and_then(|p| p.to_str())
        .map(|s| s.to_string());
    
    if let Some(rel_path) = relative_path {
        // Assuming GitHub repo URL - you may want to fetch this from git config
        let url = format!("https://github.com/remcostoeten/dotfiles/blob/master/{}", rel_path);
        Command::new("xdg-open")
            .arg(&url)
            .spawn()
            .map_err(|e| format!("Failed to open: {}", e))?;
    }
    
    Ok(())
}

#[tauri::command]
fn open_in_system_file_manager(path: String) -> Result<(), String> {
    let path_buf = PathBuf::from(&path);
    let dir = if path_buf.is_dir() {
        path_buf
    } else {
        path_buf.parent().unwrap_or(&PathBuf::from(".")).to_path_buf()
    };

    #[cfg(target_os = "linux")]
    Command::new("xdg-open")
        .arg(&dir)
        .spawn()
        .map_err(|e| format!("Failed to open: {}", e))?;

    #[cfg(target_os = "macos")]
    Command::new("open")
        .arg(&dir)
        .spawn()
        .map_err(|e| format!("Failed to open: {}", e))?;

    #[cfg(target_os = "windows")]
    Command::new("explorer")
        .arg(&dir)
        .spawn()
        .map_err(|e| format!("Failed to open: {}", e))?;

    Ok(())
}

// Run setup with real-time output streaming
#[tauri::command]
async fn run_setup(app: tauri::AppHandle) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh = dotfiles_path.join("setup.sh");
    
    eprintln!("Starting setup from path: {:?}", dotfiles_path);
    eprintln!("Setup script path: {:?}", setup_sh);
    
    run_command_with_output(&app, setup_sh, vec![]).await
}

#[tauri::command]
async fn run_setup_dry_run(app: tauri::AppHandle) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh = dotfiles_path.join("setup.sh");
    
    run_command_with_output(&app, setup_sh, vec!["--dry-run"]).await
}

#[tauri::command]
async fn run_setup_section(app: tauri::AppHandle, section: String) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh = dotfiles_path.join("setup.sh");
    
    run_command_with_output(&app, setup_sh, vec!["--dry-run-section", &section]).await
}

#[tauri::command]
async fn run_setup_dry_run_section(app: tauri::AppHandle, section: String) -> Result<(), String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let setup_sh = dotfiles_path.join("setup.sh");
    
    run_command_with_output(&app, setup_sh, vec!["--dry-run", "--dry-run-section", &section]).await
}

// Helper function to run command and stream output
async fn run_command_with_output(
    app: &tauri::AppHandle,
    script: PathBuf,
    args: Vec<&str>,
) -> Result<(), String> {
    // Log what we're about to run
    eprintln!("Running command: bash {:?} with args: {:?}", script, args);
    
    let mut child = Command::new("bash")
        .arg(&script)
        .args(&args)
        .current_dir(&script.parent().unwrap())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| format!("Failed to spawn process: {}", e))?;

    let stdout = child.stdout.take().ok_or("Failed to capture stdout")?;
    let stderr = child.stderr.take().ok_or("Failed to capture stderr")?;

    let app_clone = app.clone();
    
    // Stream stdout
    let stdout_handle = tokio::spawn(async move {
        let reader = BufReader::new(stdout);
        for line in reader.lines() {
            if let Ok(line) = line {
                let parsed = parse_output_line(&line);
                let _ = app_clone.emit("setup-output", parsed);
            }
        }
    });

    // Stream stderr
    let app_clone_err = app.clone();
    let stderr_handle = tokio::spawn(async move {
        let reader = BufReader::new(stderr);
        for line in reader.lines() {
            if let Ok(line) = line {
                let parsed = parse_output_line(&line);
                let _ = app_clone_err.emit("setup-output", parsed);
            }
        }
    });

    // Wait for process to complete
    let status = child.wait().map_err(|e| format!("Process error: {}", e))?;
    
    // Wait for output streams to finish
    let _ = stdout_handle.await;
    let _ = stderr_handle.await;

    // Emit completion event
    let exit_code = status.code().unwrap_or(-1);
    let completion_msg = if exit_code == 0 {
        OutputLine {
            line_type: "success".to_string(),
            message: format!("✓ Setup completed successfully (exit code: {})", exit_code),
            raw: String::new(),
        }
    } else {
        OutputLine {
            line_type: "error".to_string(),
            message: format!("✗ Setup failed with exit code: {}", exit_code),
            raw: String::new(),
        }
    };
    let _ = app.emit("setup-complete", completion_msg);

    if exit_code != 0 {
        return Err(format!("Process exited with code: {}", exit_code));
    }

    Ok(())
}

// ============================================================================
// BACKUP & RESTORE SYSTEM
// ============================================================================

fn get_backups_dir() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "~".to_string());
    PathBuf::from(home).join(".dotfiles-backups")
}

#[tauri::command]
fn create_backup(name: String) -> Result<Backup, String> {
    let dotfiles_path = get_dotfiles_path_internal();
    let backups_dir = get_backups_dir();
    
    // Create backups directory if it doesn't exist
    fs::create_dir_all(&backups_dir)
        .map_err(|e| format!("Failed to create backups directory: {}", e))?;
    
    let timestamp = Utc::now();
    let id = timestamp.timestamp().to_string();
    let backup_name = format!("{}_{}.tar.gz", name.replace(" ", "_"), id);
    let backup_path = backups_dir.join(&backup_name);
    
    // Create tar.gz archive
    let tar_gz = fs::File::create(&backup_path)
        .map_err(|e| format!("Failed to create backup file: {}", e))?;
    let enc = GzEncoder::new(tar_gz, Compression::default());
    let mut tar = Builder::new(enc);
    
    // Add dotfiles directory to archive
    tar.append_dir_all("dotfiles", &dotfiles_path)
        .map_err(|e| format!("Failed to create archive: {}", e))?;
    
    tar.finish()
        .map_err(|e| format!("Failed to finish archive: {}", e))?;
    
    // Get file size
    let metadata = fs::metadata(&backup_path)
        .map_err(|e| format!("Failed to get backup metadata: {}", e))?;
    
    Ok(Backup {
        id,
        name,
        timestamp: timestamp.to_rfc3339(),
        size: metadata.len(),
        path: backup_path.to_string_lossy().to_string(),
    })
}

#[tauri::command]
fn list_backups() -> Result<Vec<Backup>, String> {
    let backups_dir = get_backups_dir();
    
    if !backups_dir.exists() {
        return Ok(vec![]);
    }
    
    let mut backups = Vec::new();
    
    if let Ok(entries) = fs::read_dir(&backups_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("gz") {
                if let Ok(metadata) = fs::metadata(&path) {
                    let filename = path.file_stem()
                        .and_then(|s| s.to_str())
                        .unwrap_or("unknown")
                        .to_string();
                    
                    // Parse filename: name_timestamp.tar
                    let parts: Vec<&str> = filename.rsplitn(2, '_').collect();
                    let (id, name) = if parts.len() == 2 {
                        (parts[0].to_string(), parts[1].to_string())
                    } else {
                        (filename.clone(), filename.clone())
                    };
                    
                    if let Ok(modified) = metadata.modified() {
                        let datetime: DateTime<Utc> = modified.into();
                        backups.push(Backup {
                            id,
                            name: name.replace("_", " "),
                            timestamp: datetime.to_rfc3339(),
                            size: metadata.len(),
                            path: path.to_string_lossy().to_string(),
                        });
                    }
                }
            }
        }
    }
    
    // Sort by timestamp (newest first)
    backups.sort_by(|a, b| b.timestamp.cmp(&a.timestamp));
    
    Ok(backups)
}

#[tauri::command]
fn restore_backup(backup_id: String) -> Result<String, String> {
    let backups_dir = get_backups_dir();
    let dotfiles_path = get_dotfiles_path_internal();
    
    // Find backup file
    let backup_file = if let Ok(entries) = fs::read_dir(&backups_dir) {
        entries.flatten()
            .find(|e| {
                e.path().file_name()
                    .and_then(|n| n.to_str())
                    .map(|n| n.contains(&backup_id))
                    .unwrap_or(false)
            })
            .map(|e| e.path())
    } else {
        None
    };
    
    let backup_path = backup_file.ok_or("Backup not found")?;
    
    // Extract archive
    let tar_gz = fs::File::open(&backup_path)
        .map_err(|e| format!("Failed to open backup: {}", e))?;
    let dec = flate2::read::GzDecoder::new(tar_gz);
    let mut archive = tar::Archive::new(dec);
    
    // Create temporary restore directory
    let temp_restore = dotfiles_path.parent()
        .ok_or("Invalid dotfiles path")?
        .join("dotfiles_restore_temp");
    
    fs::create_dir_all(&temp_restore)
        .map_err(|e| format!("Failed to create temp directory: {}", e))?;
    
    archive.unpack(&temp_restore)
        .map_err(|e| format!("Failed to extract backup: {}", e))?;
    
    Ok(format!("Backup extracted to: {}", temp_restore.to_string_lossy()))
}

#[tauri::command]
fn delete_backup(backup_id: String) -> Result<(), String> {
    let backups_dir = get_backups_dir();
    
    if let Ok(entries) = fs::read_dir(&backups_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.file_name()
                .and_then(|n| n.to_str())
                .map(|n| n.contains(&backup_id))
                .unwrap_or(false)
            {
                fs::remove_file(&path)
                    .map_err(|e| format!("Failed to delete backup: {}", e))?;
                return Ok(());
            }
        }
    }
    
    Err("Backup not found".to_string())
}

// ============================================================================
// PACKAGE STATUS CHECKING
// ============================================================================

#[tauri::command]
fn check_package_installed(package_name: String) -> Result<PackageStatus, String> {
    // Extract actual package name from format "package:Display Name"
    let actual_name = if let Some(colon_pos) = package_name.find(':') {
        &package_name[..colon_pos]
    } else {
        &package_name
    };
    
    let display_name = if let Some(colon_pos) = package_name.find(':') {
        package_name[colon_pos + 1..].trim().to_string()
    } else {
        package_name.clone()
    };
    
    // Check if command exists
    let installed = Command::new("which")
        .arg(actual_name)
        .output()
        .map(|output| output.status.success())
        .unwrap_or(false);
    
    // Try to get version
    let version = if installed {
        Command::new(actual_name)
            .arg("--version")
            .output()
            .ok()
            .and_then(|output| {
                if output.status.success() {
                    String::from_utf8(output.stdout).ok()
                        .map(|s| s.lines().next().unwrap_or("").to_string())
                } else {
                    None
                }
            })
    } else {
        None
    };
    
    Ok(PackageStatus {
        name: actual_name.to_string(),
        display_name,
        installed,
        version,
    })
}

#[tauri::command]
fn get_setup_sections() -> Result<Vec<SetupSection>, String> {
    let arrays = parse_setup_sh();
    let mut sections = Vec::new();
    
    for array in arrays {
        let mut package_statuses = Vec::new();
        
        for pkg in &array.packages {
            if let Ok(status) = check_package_installed(pkg.clone()) {
                package_statuses.push(status);
            }
        }
        
        // Estimate time based on package count (2 seconds per package)
        let estimated_time = (package_statuses.len() as u32) * 2;
        
        sections.push(SetupSection {
            name: array.name.clone(),
            description: format!("Install {} packages", package_statuses.len()),
            packages: package_statuses,
            estimated_time,
        });
    }
    
    Ok(sections)
}

// ============================================================================
// TERMINAL COMMANDS
// ============================================================================

#[tauri::command]
fn execute_command(command: String, args: Vec<String>) -> Result<String, String> {
    let output = Command::new(&command)
        .args(&args)
        .current_dir(get_dotfiles_path_internal())
        .output()
        .map_err(|e| format!("Failed to execute command: {}", e))?;
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    
    Ok(format!("{}{}", stdout, stderr))
}

#[tauri::command]
fn get_shell_env() -> Result<String, String> {
    Ok(std::env::var("SHELL").unwrap_or_else(|_| "/bin/bash".to_string()))
}

use tauri::{Manager, menu::{Menu, MenuItem}, tray::TrayIconBuilder};

fn main() {
    tauri::Builder::default()
        .plugin(tauri_plugin_shell::init())
        .setup(|app| {
            // Build system tray menu
            let quit_i = MenuItem::with_id(app, "quit", "Quit", true, None::<&str>)?;
            let show_i = MenuItem::with_id(app, "show", "Show Window", true, None::<&str>)?;
            let backup_i = MenuItem::with_id(app, "backup", "Quick Backup", true, None::<&str>)?;
            let git_status_i = MenuItem::with_id(app, "git_status", "Git Status", true, None::<&str>)?;
            let terminal_i = MenuItem::with_id(app, "terminal", "Open Terminal", true, None::<&str>)?;
            
            let menu = Menu::with_items(app, &[
                &show_i,
                &terminal_i,
                &backup_i,
                &git_status_i,
                &quit_i,
            ])?;

            // Create system tray
            let _tray = TrayIconBuilder::new()
                .menu(&menu)
                .on_menu_event(|app, event| {
                    match event.id.as_ref() {
                        "quit" => {
                            app.exit(0);
                        }
                        "show" => {
                            if let Some(window) = app.get_webview_window("main") {
                                let _ = window.show();
                                let _ = window.set_focus();
                            }
                        }
                        "backup" => {
                            // Trigger quick backup
                            if let Some(window) = app.get_webview_window("main") {
                                let _ = window.emit("quick-action", "backup");
                                let _ = window.show();
                                let _ = window.set_focus();
                            }
                        }
                        "git_status" => {
                            // Show git status
                            if let Some(window) = app.get_webview_window("main") {
                                let _ = window.emit("quick-action", "git-status");
                                let _ = window.show();
                                let _ = window.set_focus();
                            }
                        }
                        "terminal" => {
                            // Open terminal tab
                            if let Some(window) = app.get_webview_window("main") {
                                let _ = window.emit("quick-action", "terminal");
                                let _ = window.show();
                                let _ = window.set_focus();
                            }
                        }
                        _ => {}
                    }
                })
                .build(app)?;

            Ok(())
        })
        .invoke_handler(tauri::generate_handler![
            get_package_arrays,
            add_package_to_array,
            remove_package_from_array,
            get_aliases,
            get_functions,
            read_file,
            write_file,
            list_files,
            get_dotfiles_path,
            open_in_github,
            open_in_system_file_manager,
            get_git_status,
            run_git_command,
            run_setup,
            run_setup_dry_run,
            run_setup_section,
            run_setup_dry_run_section,
            // Backup & Restore
            create_backup,
            list_backups,
            restore_backup,
            delete_backup,
            // Interactive Setup
            check_package_installed,
            get_setup_sections,
            // Terminal
            execute_command,
            get_shell_env,
        ])
        .run(tauri::generate_context!())
        .expect("error while running tauri application");
}
