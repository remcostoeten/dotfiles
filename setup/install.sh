...
# This script installs prerequisites and sets up the Setup system
...
INSTALL_DIR="${HOME}/.local/share/setup"

# Print functions
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║${NC} ${GREEN}Setup Bootstrap${NC} ${BLUE}                                         ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
install_dependencies() {
    print_status "Installing Setup dependencies..."
...
verify_setup() {
    ...
    if [ -d "$SETUP_DIR" ]; then
        ...
        print_error "Setup directory not found"
        ((errors++))
    fi

    # Check if dependencies are installed
    if [ -f "$SETUP_DIR/package.json" ] && [ -d "$SETUP_DIR/node_modules" ]; then
...
if command -v bun >/dev/null 2>&1; then
    ...
    node_modules/.bin/bun run setup "$@"
else
...
EOF
    ...
    print_success "Launcher script created: setup"
}
...
show_next_steps() {
    ...
    echo "2. Run the setup: setup"
    echo "   Or: cd ~/.config/dotfiles/setup && bun run setup"
    echo ""
