    ...
    try {
      ...
      switch (pkg.method) {
        ...
        case "cargo":
          ...
          break;
        case "script":
          // Execute local script files
          if (options.verbose) {
            console.log(`🔧 Executing local script: ${pkg.extra}`);
          }
          result = await executeLocalScript(pkg.extra || "", options.verbose);
          break;
        case "legacy":
          // Execute legacy setup.sh
          if (options.verbose) {
            console.log(`🔄 Running legacy setup: ${pkg.extra}`);
          }
          result = await executeLegacySetup(pkg.extra || "", options.verbose);
          break;
        default:
