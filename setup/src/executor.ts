...
export async function executeLocalScript(
  ...
  const fullPath = scriptPath.startsWith('/') ? scriptPath : `${__dirname}/${scriptPath}`;
  return executeCommand(`bash "${fullPath}"`, verbose);
}

/**
 * Execute legacy setup.sh with specific arguments
 */
export async function executeLegacySetup(
  args: string = "",
  verbose: boolean = false
): Promise<CommandResult> {
  const setupPath = `${__dirname}/../setup.sh`;
  return executeCommand(`bash "${setupPath}" ${args}`, verbose);
}
