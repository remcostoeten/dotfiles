...
export async function setupDotfiles(): Promise<DotfilesSetupResult> {
  ...
  // 11. Setup config app symlinks (kitty, neovim, wezterm, hyprland, waybar)
...
/**
 * Setup config app symlinks (kitty, neovim, wezterm, hyprland, waybar, etc.)
 */
async function setupConfigApps(): Promise<DotfilesSetupResult> {
  ...
  const configApps: Record<string, string> = {
    ...
    wezterm: `${process.env.HOME}/.config/wezterm`,
