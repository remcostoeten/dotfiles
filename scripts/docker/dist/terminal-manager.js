...
export class TerminalManager {
    ...
    applyFilter() {
        if (!this.searchQuery) {
            ...
            this.filteredContainers = this.containers.filter(c => c.name.toLowerCase().includes(query) || c.image.toLowerCase().includes(query));
        }
    ...
    async copyContainerEnv(container) {
        if (container.status !== 'running') {
    ...
    async execInContainer(container) {
        clearScreen();
        UI.printHeader();
    ...
    async restartAllRunning() {
        ...
        if (confirm.toLowerCase() === 'yes') {
            ...
            const result = await DockerUtils.performBulkOperation(running.map(c => c.id), 'stop');
            if (result.ok) {
    ...
    async startAllStopped() {
        ...
        const result = await DockerUtils.performBulkOperation(stopped.map(c => c.id), 'start');
        if (result.ok) {
    ...
    async removeAllStopped() {
        ...
        if (confirm.toLowerCase() === 'yes') {
            ...
            const result = await DockerUtils.performBulkOperation(stopped.map(c => c.id), 'remove');
            if (result.ok) {
    ...
    async saveDockerComposeToFile(content) {
        try {
