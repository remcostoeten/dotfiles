...
import type { TContainer, TContainerStats, TResult, TContainerStatus } from './types.js';
    ...
    return lines.slice(1).map(line => {
        ...
        if (parts.length < 7) {
            ...
            return {
                ...
                image: image || 'unknown',
                status: (status.toLowerCase().includes('up') ? 'running' : 'exited') as TContainerStatus,
                ports: [],
 ...
 */
export async function execInContainer(id: string, command: string): Promise<TResult<string, string>> {
    try {
 ...
 */
export async function getContainerEnv(id: string): Promise<TResult<Record<string, string>, string>> {
    try {
...
): Promise<TResult<{ id: string; connectionString: string }, string>> {
    try {
 ...
 */
export async function searchContainers(query: string): Promise<TResult<TContainer[], string>> {
    try {
        ...
        const filtered = allContainers.value.filter(
            container =>
 ...
 */
export async function restartContainer(id: string): Promise<TResult<void, string>> {
    try {
 ...
 */
export function generateEnvironmentVariables(containers: TContainer[], isVite: boolean = false): string {
 ...
 */
export function generateDockerCompose(containers: TContainer[]): string {
    const services: string[] = [];
    const volumes: string[] = [];
    const networks: string[] = ['default'];
