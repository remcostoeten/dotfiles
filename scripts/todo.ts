#!/usr/bin/env bun
// DOCSTRING: Cross-platform CLI todo/task manager with interactive TUI and persistent storage
...
interface Task {
    ...
    reminderOffsets: number[]; // Minutes before due date
    notificationsSent: {
        overdue?: boolean;
        reminders: number[]; // Track which reminder offsets have been sent
    };
}
...
const DOTFILES_ROOT = process.env.HOME + '/.config/dotfiles';
const TODOS_DIR = join(DOTFILES_ROOT, 'todos');
const TASKS_FILE = join(TODOS_DIR, 'tasks.json');
const CONFIG_FILE = join(TODOS_DIR, 'config.json');
...
function parseTime(input: string): number | null {
    ...
    // "in X" patterns: "in 2h", "in 30min", "in 2 days"
    ...
    // Clock times: "12am", "12 pm", "3pm", "3 pm"
    ...
    // Date + time: "2025-11-10 14:30" or "2025-11-10"
...
function isUpcoming(timestamp: number): boolean {
    const now = Date.now();
    const diff = timestamp - now;
    return diff > 0 && diff < 30 * 60 * 1000; // Less than 30 minutes
}
...
function sendNotification(title: string, message: string): void {
    ...
    try {
        ...
        // Fallback: try other notification methods if needed
    }
}

function checkAndSendNotifications(tasks: Task[]): void {
    const now = Date.now();
    ...
    for (const task of tasks) {
        ...
        for (const offset of task.reminderOffsets) {
            ...
            if (now >= reminderTime && !task.notificationsSent.reminders.includes(offset)) {
                sendNotification(
                    'Task Reminder',
                    `${task.description} - due in ${offset} minutes`
                );
...
function getUrgencyColor(timestamp?: number): string {
    if (!timestamp) return COLORS.SUBTEXT0;
...
function formatTaskForDisplay(task: Task, showId: boolean = false): string {
    const parts: string[] = [];
...
function displayTasksForShell(tasks: Task[]): void {
...
class TodoTUI {
    ...
    private currentView: 'main' | 'create' | 'delete' | 'edit' | 'help' = 'main';
    ...
    renderTaskList(title: string, showActions: boolean = true): void {
        this.clearScreen();
    ...
    async editTaskDetails(task: Task): Promise<void> {
        this.clearScreen();
    ...
    async handleMainMenuSelection(): Promise<void> {
...
async function handleCLI(args: string[]): Promise<void> {
    const tasks = loadTasks();
    const config = loadConfig();
    
    if (args.length === 0) {
        // Non-interactive mode - show tasks for shell startup
    ...
    if (command.startsWith('"') || !command.startsWith('-')) {
        // Parse: todo "Description" time [--r reminders]
        let description = '';
        let timeStr = '';
        let reminderOffsets: number[] = [];
        
        let i = 0;
        // Find description (quoted or first arg)
